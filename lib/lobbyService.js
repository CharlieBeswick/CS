  ...
  return {
    ...
    round: round
      ? {
          ...
          finalRotation: round.finalRotation, // Server-calculated final rotation for synchronization
...
async function startSpinning(lobby) {
  ...
  // Calculate deterministic final rotation angle
  // Each segment is 18 degrees (360/20)
  // Segment 1 starts at -90 degrees (top), segment 2 at -72, etc.
  // The arrow is fixed at the top (0 degrees / -90 degrees in original coordinates)
  // We want the winning segment to be centered under the arrow
  // 
  // In the original wheel (no rotation):
  // - Segment 1 center is at -90 + 9 = -81 degrees
  // - Segment 2 center is at -90 + 18 + 9 = -63 degrees
  // - Segment N center is at -90 + (N-1)*18 + 9 degrees
  //
  // To center segment N at the top (arrow at 0 degrees):
  // - We need to rotate the wheel so segment N's center moves to 0 degrees
  // - If segment N center is at angle A, we rotate by -A to bring it to 0
  // - But since rotation is clockwise, we actually rotate by +A
  const segmentAngle = 360 / wheelSegments.length; // 18 degrees
  const segmentCenterOffset = segmentAngle / 2; // 9 degrees (center of segment)
  // Winning segment's center angle in the wheel's original position (in degrees, where -90 = top)
  const winningSegmentCenterAngle = -90 + (winningSegment - 1) * segmentAngle + segmentCenterOffset;
  // To center it at the top (0 degrees), rotate by the negative of this angle
  // Add extra rotations from seed for visual variety
  const extraRotations = (parseInt(seed.slice(8, 12), 16) % 10) + 5; // 5-14 full rotations
  const extraOffset = (parseInt(seed.slice(12, 16), 16) % 360); // 0-359 degrees
  // Final rotation: extra spins + offset - winningSegmentCenterAngle
  // This centers the winning segment at the top
  const finalRotation = (extraRotations * 360) + extraOffset - winningSegmentCenterAngle;
  ...
  await prisma.lobbyRound.upsert({
    ...
    update: {
      ...
      wheelSegments,
      seed,
      finalRotation: normalizedFinalRotation,
      winningSegment,
...
/**
 * Resolve lobby with winning segment from frontend
 * Called when the wheel stops (speed < 0.2)
 */
async function resolveLobbyWithWinningSegment({ lobbyId, userId, winningSegment, winningNumber }) {
  // Use a transaction to prevent race conditions when multiple clients resolve simultaneously
  return prisma.$transaction(async (tx) => {
    const lobby = await tx.tierLobby.findUnique({
      where: { id: lobbyId },
      include: { round: true },
    ...
    if (lobby.status !== 'SPINNING') {
      // If already resolved, return the lobby without error (idempotent)
      if (lobby.status === 'RESOLVED') {
        return serializeLobby(await loadLobbyGraph(lobbyId), userId);
      }
      throw new Error(`Lobby is not in SPINNING state (current: ${lobby.status})`);
    }
    
    // Verify user is in the lobby
    const player = await tx.lobbyPlayer.findFirst({
    ...
    // Use backend's pre-calculated winning segment (from seed) - this is the source of truth
    // The frontend's calculated segment might differ due to floating point precision
    const backendWinningSegment = lobby.round?.winningSegment;
    const segmentToUse = backendWinningSegment || winningSegment;
    const numberToUse = backendWinningSegment || (winningNumber || winningSegment);
    
    // Update round with winning segment/number
    await tx.lobbyRound.updateMany({
      where: { lobbyId },
      data: {
        winningSegment: segmentToUse,
        winningNumber: numberToUse,
        resolvedAt: now,
        spinCompletedAt: now,
      },
    });
    ...
    console.log('[lobby] resolved with winning segment from frontend', {
      lobbyId,
      winningSegment: segmentToUse,
      winningNumber: numberToUse,
      usedBackendSegment: !!backendWinningSegment,
    });
    
    await recordGameHistory(lobbyId);
    return serializeLobby(await loadLobbyGraph(lobbyId), userId);
  });
}
...
async function advanceLobbyState(lobbyId) {
  ...
  // Don't auto-resolve SPINNING lobbies - wait for frontend to report winning segment
  // The frontend will call resolveLobbyWithWinningSegment when wheel speed < 0.2
  // if (lobby.status === 'SPINNING' && lobby.round?.spinCompletedAt && now >= lobby.round.spinCompletedAt) {
