...
async function startSpinning(lobby) {
  ...
  // We want the winning segment to be centered under the arrow (at 0 degrees / top)
  // So we rotate the wheel so the winning segment's center is at the top
  ...
  // Winning segment's center angle in the wheel's original position
  const winningSegmentCenterAngle = -90 + (winningSegment - 1) * segmentAngle + segmentCenterOffset;
  // To center it at the top (0 degrees), we need to rotate by -winningSegmentCenterAngle
  // Add some extra rotation from the seed for visual variety (multiple full rotations + offset)
...
async function resolveLobbyWithWinningSegment({ lobbyId, userId, winningSegment, winningNumber }) {
  ...
  // Update round with winning segment/number from frontend
  await prisma.lobbyRound.updateMany({
    where: { lobbyId },
    data: {
