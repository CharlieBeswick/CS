...
const { getNextTier } = require('../config/tickets');
...
function formatPlayer(player, lobbyStatus, viewerId) {
  ...
  return {
    ...
    rpmAvatarId: player.user?.rpmAvatarId || null,
    rpmAvatarUrl: player.user?.rpmAvatarUrl || null,
...
async function resolveLobby(lobby) {
  // Load full lobby with players and round
  const fullLobby = await prisma.tierLobby.findUnique({
    where: { id: lobby.id },
    include: {
  ...
  if (!fullLobby || !fullLobby.round) {
    throw new Error('Lobby or round not found');
  }

  const now = new Date();
  const winningNumber = fullLobby.round.winningNumber;

  // Find the winning player (player whose luckyNumber matches winningNumber)
  const winningPlayer = fullLobby.players.find(
    player => player.luckyNumber === winningNumber
  );

  // Award tickets to winner if found
  if (winningPlayer) {
    const nextTier = getNextTier(fullLobby.tier);
    if (nextTier) {
      // Award 1 next-tier ticket to the winner
      // Works regardless of player count (even with 3 players for testing)
      await addTickets(
        ...
        'LOBBY_WIN',
        {
          ...
          luckyNumber: winningPlayer.luckyNumber,
          gameNumber: null, // Will be set when history is recorded
        }
      );

      console.log('[lobby] awarded winner tickets', {
        lobbyId: fullLobby.id,
      ...
      console.log('[lobby] winner found but no next tier (DIAMOND tier)', {
        lobbyId: fullLobby.id,
        winnerUserId: winningPlayer.userId,
        tier: fullLobby.tier,
      });
    ...
    console.warn('[lobby] no winning player found', {
      lobbyId: fullLobby.id,
      winningNumber: winningNumber,
      players: fullLobby.players.map(p => ({ userId: p.userId, luckyNumber: p.luckyNumber })),
    });
  }

  // Update round with resolvedAt and winningPlayerId
  await prisma.lobbyRound.updateMany({
    where: { lobbyId: fullLobby.id },
    data: {
      resolvedAt: now,
      winningPlayerId: winningPlayer?.id || null,
    },
  });

  // Update lobby status
  const updated = await prisma.tierLobby.update({
    where: { id: fullLobby.id },
    data: {
      status: 'RESOLVED',
      resolvedAt: now,
    },
  ...
  console.log('[lobby] resolved', {
    lobbyId: fullLobby.id,
    winningNumber: winningNumber,
    hasWinner: !!winningPlayer,
  });

  await recordGameHistory(fullLobby.id);
  return updated;
}
